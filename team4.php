...
if (isset($current_quality)) {
  ...
  $_r = new Rounds();
  $r_quantity  = $_r->get_by_where1('team','team4');
  $round = 'quantity_round'.$_SESSION['Round'];
  $current_quantity = $r_quantity[0]['quality_round1'];

  require_once 'lib/database.php';
  $_db = new Database();
  $q = "SELECT SUM(quality_round1+quality_round2+quality_round3+quality_round4+quality_round5+quality_round6+quality_round7+quality_round8+quality_round9+quality_round10+quality_round11+quality_round12+quality_round13+quality_round14+quality_round15) FROM rounds WHERE team = 'team4'";
  $_sum = $_db->get_by_query($q);
  $TotalQuality = $_sum[0][0];
  //////////////////////////////
  $q = "SELECT SUM(quantity_round1+quantity_round2+quantity_round3+quantity_round4+quantity_round5+quantity_round6+quantity_round7+quantity_round8+quantity_round9+quantity_round10+quantity_round11+quantity_round12+quantity_round13+quantity_round14+quantity_round15) FROM rounds WHERE team = 'team4'";
  $_sum = $_db->get_by_query($q);
  $TotalQuantity = $_sum[0][0];
  //////////////////////////////////////
  $q = "SELECT SUM(bonus_round1+bonus_round2+bonus_round3+bonus_round4+bonus_round5+bonus_round6+bonus_round7+bonus_round8+bonus_round9+bonus_round10+bonus_round11+bonus_round12+bonus_round13+bonus_round14+bonus_round15) FROM rounds WHERE team = 'team4'";
  $_sum = $_db->get_by_query($q);
  $TotalBonus = $_sum[0][0];
  ////////////////////////////////////////////
...
<head>
  ...
  <script type="text/javascript" src="http://code.jquery.com/jquery-1.10.2.js"></script>
   <script src="js/timer_team4.js"></script>
   <script>
    function ajaxCall() {
      $.ajax({
         url: "sessions_team4.php", 
...
<div style=" background-color: #eff4ff; background-color: #eff4ff;
    ...
    border-radius: 4px;" align="center">Team 4</h2>


<div style="float:left; padding-left: 20px;     margin-top: 20px;">

<form name="saveRoundTeam4" method="post" action="funcs/counter_functions2.php">
<?php 

 if ( $_SESSION['RoundCompleate_team4']== true) {
  $_r = new Rounds();
  $r  = $_r->get_by_where1('team','team4');
  $round = 'quality_round'.$_SESSION['Round'];
  $current_quality = $r[0][$round];
  echo '<div id="div1"></div>';
     echo '<form action="counter_functions2.php" name="add_bonus_team4" method="post" >';
        ...
        echo '<Button name="add_bonus_team4" id="button-start_hold" style="font-weight: 700; 
    ...
    echo '<div class="wrapper"   style="float:left;">
      ...
      <button name="saveRoundTeam4" style=" margin-right: 10px;margin-top: -12px;" onclick="getTime()"  class="btn btn-warning">Stop</button>
...
<?php
  ...
  if ($_SESSION['Round'] >= 1) {
    if (isset($_SESSION['Team4QuantityScoreRound'])) {
  ...
  <div style="margin-top: -10px; background-color: #037fdb; ">
   <p style="color:white;">TOTAL '.$TotalRound.'</p>';
     ...
     <tr align="center">
       ...
       <td style="background-color:white" width="28%">'.$_SESSION['Team4QuantityTotal'].'</td>
       <td width="2%"></td>
  ...
  <td style="background-color:white" width="20%"> 
    ...
    if ($_SESSION['RoundCompleate_team4'] == true && $_SESSION['RoundQualityCompleate_team4'] == true) {
      echo $_SESSION['Team4BonusScoreRound'];
    }
    elseif ($_SESSION['RoundCompleate_team4'] == true ) {
      echo $_SESSION['Team4BonusScoreRound'];
...
echo '</table></div></div>';
  ...
  <tr align="center">
    <td align="right" style=" padding-right: 13px;border-right-style: groove;">
      '.$_SESSION['Team4QuantityScoreRound'].'
    </td>
    <td width="70%" style=" color: #c66464; font-size: 18px;">
      '.$_SESSION['Team4QuantityTotal'].'
    </td>
...
<?php 
  if (isset($_SESSION['RoundCompleate_team4'])) {
    if ($_SESSION['RoundCompleate_team4'] == true) {
      echo '<br>';
    }
    else{
