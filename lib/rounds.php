<?php
  ...
  class Rounds{
    ...
    public function get_by_round($round)
    {
        ...
        {
          $db = new Database();
          echo $q = "SELECT * FROM ".$this->table_name." WHERE $round !=''";
    ...
    public function updateInc($fileds,$values,$counter,$select,$id)
    {
        $message ='';
        $this->con_state = $this->Check_Connection();
        if ($this->con_state) 
        {
          $_db = new Database();
          $_db->updateInc($this->table_name,$fileds,$values,$counter,$select,$id);
        } 
